...
function ctx_sub_menu( $block_content, $block, $instance ) {
    ...
    if ( 'core/navigation-submenu' === $block['blockName'] ) {
		...
		$block_content .= "<a href='" . $block['attrs']['url'] . "'>" . $block['attrs']['label'] . "</a><button class='ctx-menu__item-arrow'><i class='material-icons'>keyboard_arrow_down</i></button></span>";
		...
		if ( ! empty( $block['innerBlocks'] ) ) {
			...
			foreach ( $block['innerBlocks'] as $inner_block ) {
				...
				$block_content .= "<a href='" . $inner_block['attrs']['url'] . "'>" . $inner_block['attrs']['label'] . "</a>";
				$block_content .= "</li>";
			}
			$block_content .= "</ul>";
		}
